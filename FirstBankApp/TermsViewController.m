...
// #import "RegistrationCodeViewController.h" // No longer needed here as TermsViewController doesn't directly navigate to it
...
- (void)viewDidLoad {
    [super viewDidLoad];
    // Hide the back button in the navigation bar for this view controller
...
- (IBAction)acceptButtonTapped:(UIButton *)sender {
    // 1. Save the state that terms have been accepted
    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"termsAccepted"];
    [[NSUserDefaults standardUserDefaults] synchronize]; // Save immediately

    // 2. Dismiss the Terms and Conditions screen
    // The completion block ensures that the delegate method is called ONLY AFTER
    // this view controller has finished its dismissal animation and is no longer
    // in the view hierarchy.
    [self dismissViewControllerAnimated:YES completion:^{
        // 3. Notify the delegate (OptionsOnboardigViewController)
        // Check if the delegate exists and responds to the selector
        if ([self.delegate respondsToSelector:@selector(termViewVCDidAcceptTerms:forDestination:)]) {
            // Corrected method call: Use 'termViewVCDidAcceptTerms' as defined in the protocol
            [self.delegate termViewVCDidAcceptTerms:self forDestination:self.destinationType];
        } else {
            NSLog(@"Error: TermsViewController delegate not set or does not respond to selector.");
        }
    }];
}
